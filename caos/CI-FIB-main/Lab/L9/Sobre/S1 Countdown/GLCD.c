...
//  Set all pixels in ri to re pages and ci to ce columns 
// ri,re [0:7] ci,ce [0:127]
void setGLCD(byte ri, byte re, byte ci, byte ce) 
{
   int i,j;
...
void putchGLCD(byte p, byte y, char c, int n) 
{
   int fontPos = (c-' ')*5;
   int i;
   y = y*5;
   ...
   for (i = 0; i < 5; ++i)
   {
      if (n == 0) aux = font5x7[fontPos];
      else aux = ~font5x7[fontPos];
      writeByte (p, y, aux);
      ++y;
      ++fontPos;
   }
...
// Els caracters estan definits en la taula font5x7[], al fitxer ascii.h
void writeNum (byte p, byte y, int i, int n) {
   //int page = 0;
   //int y = 0;
   int aux;
   char arr[32];
